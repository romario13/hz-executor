...
public class Test7 {
    ...
    public static class CallableTask implements Callable, Serializable, HazelcastInstanceAware {

        private HazelcastInstance hazelcastInstance;

        private int key = -1;

        public CallableTask(int key) {
            this.key = key;
        }
    ...
    private static void run() throws InterruptedException {
        ...
        Future futures[] = new Future[TASK_QUANTITY];
        ...
        while (i++ < TASK_QUANTITY) {
            ...
            CallableTask task = new CallableTask(key);

            futures[i - 1] = executorService.submitToKeyOwner(new CallableTask(key), key);
        }
        ...
        int executionExceptionCounter = 0;
        ...
        logger.info("Canceled tasks: " + canceledCounter + " execution exceptions: " + executionExceptionCounter + " " +
                "rejected tasks: " + rejectedCounter);
    }
